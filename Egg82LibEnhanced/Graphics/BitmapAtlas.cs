...
namespace Egg82LibEnhanced.Graphics {
	public class BitmapAtlas : IDisposable {
		...
		//constructor
		public BitmapAtlas(Bitmap bitmap, XmlDocument atlasXml) {
			if (bitmap == null) {
		...
		public BitmapAtlas(Bitmap bitmap, int columns, int rows) {
			if (bitmap == null) {
				throw new ArgumentNullException("bitmap");
			}
			if (rows <= 0) {
		...
		public bool HasBitmap(string name) {
			if (name == null) {
				throw new ArgumentNullException("name");
			}
		...
		public void AddRegion(string name, Rectangle region, bool rotated = false) {
			...
			if (subBitmaps.ContainsKey(name)) {
				subBitmaps[name] = TextureUtil.GetRegion(atlasBitmap, region, (rotated) ? -90.0d : 0.0d);
			} else {
				subBitmaps.Add(name, TextureUtil.GetRegion(atlasBitmap, region, (rotated) ? -90.0d : 0.0d));
			}
		}
